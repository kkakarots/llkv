...
export default React.memo(function LayoutRow({
  ...
  const confirmRevert = useCallback(async () => {
    const response = await confirm({
      title: multiSelection ? t("RevertLayouts") : t("RevertLayout", { layoutName: layout.name }),
      prompt: t("ChangesWillBeDiscarded"),
      ok: t("DiscardChanges"),
      variant: "danger",
    });
    if (response !== "ok") {
  ...
  const confirmDelete = useCallback(() => {
    const layoutWarning =
      !multiSelection && layoutIsShared(layout) ? t("OrganizationMembersWarning") : "";
    const prompt = `${layoutWarning}${t("ThisActionCannotBeUndone")}`;
    const title = multiSelection ? t("DeleteSelectedLayouts") : `${t("Delete")} "${layout.name}"?`;
    void confirm({ title, prompt, ok: t("Delete"), variant: "danger" }).then((response) => {
      ...
      >
        <ListItemText disableTypography>
          <TextField
            ...
            onBlur={onBlur}
            fullWidth
            style={{ font: "inherit", display: editingName ? "inline" : "none" }}
            size="small"
            variant="filled"
          />
          <Typography
