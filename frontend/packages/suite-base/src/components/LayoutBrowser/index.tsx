...
import { ClassicLayoutDialog } from "@lichtblick/suite-base/components/ClassicLayoutDialog";
...
export default function LayoutBrowser({
  ...
  // 经典布局弹窗状态
  const [classicLayoutDialogOpen, setClassicLayoutDialogOpen] = useState(false);
  ...
  // 打开经典布局选择弹窗
  const handleSelectClassicLayout = useCallback(() => {
    setClassicLayoutDialogOpen(true);
  }, []);

  // 关闭经典布局弹窗
  const handleCloseClassicLayoutDialog = useCallback(() => {
    setClassicLayoutDialogOpen(false);
    menuClose?.();
  }, [menuClose]);

  // 包装 importLayout 以在导入后关闭菜单
  const handleImportLayout = useCallback(async () => {
    await importLayout();
    menuClose?.();
  }, [importLayout, menuClose]);
  ...
  // 包装 onSelectLayout 以在选择后关闭菜单
  const handleSelectLayout = useCallback(
    async (layout: Layout) => {
      await onSelectLayout(layout);
      menuClose?.();
    },
    [onSelectLayout, menuClose],
  );
  ...
  return (
    <>
          ...
          {enableNewTopNav && (
            <>
              <List className={classes.actionList} disablePadding>
                ...
                <ListItem disablePadding>
                  <ListItemButton onClick={handleImportLayout}>
                    <ListItemText disableTypography>{t("ImportFromFile")}</ListItemText>
                  </ListItemButton>
                </ListItem>
                <ListItem disablePadding>
                  <ListItemButton onClick={handleSelectClassicLayout}>
                    <ListItemText disableTypography>{t("SelectClassicLayout")}</ListItemText>
                  </ListItemButton>
                </ListItem>
              </List>
      ...
      {/* 经典布局选择弹窗 */}
      <ClassicLayoutDialog
        open={classicLayoutDialogOpen}
        onClose={handleCloseClassicLayoutDialog}
      />
    </>
  );
}
