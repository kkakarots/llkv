...
type VisibilityOptionsConfig = { includeMatchSubstring: boolean };
const SelectVisibilityFilterOptions: (
  t: TFunction<"settingsEditor">,
  opts: VisibilityOptionsConfig,
) => {
  label: string;
  value: SelectVisibilityFilterValue;
}[] = (t, opts) => {
  const base = [
    { label: t("listAll"), value: "all" as const },
    { label: t("listVisible"), value: "visible" as const },
    { label: t("listInvisible"), value: "invisible" as const },
  ];
  return opts.includeMatchSubstring
    ? [...base, { label: t("listMatchSubstring"), value: "matchSubstring" as const }]
    : base;
};
...
const getSelectVisibilityFilterField = (
  ...
  ({
    ...
    options: SelectVisibilityFilterOptions(t, opts),
  }) as const;

/**
 * Extract the path segment between the first pair of /.../ in a string
 * @param input The input string to extract from
 * @returns The path segment between /.../, or empty string if not found
 */
function extractPathSegment(input: string): string {
...
function NodeEditorComponent(props: Readonly<NodeEditorProps>): React.JSX.Element {
  ...
  const matchSubstring = props.settings?.filterMatchSubstring;
  ...
  const filterFn = useMemo(() => {
    ...
    if (state.visibilityFilter === "matchSubstring" && typeof matchSubstring === "string") {
      return (child: Immutable<SettingsTreeNode>) => {
        const labelMatches =
          extractPathSegment(child.label ?? "") === extractPathSegment(matchSubstring) ||
          (child.label ?? "").includes(matchSubstring);
        return labelMatches;
      };
    }
    return undefined;
  }, [state.visibilityFilter, matchSubstring]);
  ...
  const filteredNodes = useMemo(() => {
    ...
    if (state.visibilityFilter === "matchSubstring" && typeof matchSubstring === "string") {
      return preparedNodes.filter(
        ([key, child]) => key.includes(matchSubstring) || filterFn(child),
      );
    }
    return preparedNodes.filter(([, child]) => filterFn(child));
  }, [preparedNodes, filterFn, state.visibilityFilter, matchSubstring]);
      ...
      {state.open && selectVisibilityFilterEnabled && hasChildren && (
        <>
          ...
          <FieldEditor
            ...
            field={{
              ...getSelectVisibilityFilterField(t, {
                includeMatchSubstring: typeof matchSubstring === "string",
              }),
