...
export class ImageAnnotations extends THREE.Object3D {
  ...
  public settingsNodes(): SettingsTreeEntry[] {
    ...
    entries.push({
      ...
      node: {
        ...
        filterMatchSubstring: config.imageTopic,
  ...
  /**
   * Updates visibility of annotation topics based on the current image topic.
   * This is called when the image topic changes to automatically show matching topics
   * and hide non-matching ones, but preserves user's manual visibility settings.
   */
  public updateVisibilityForImageTopic(imageTopic: string | undefined): void {
    if (!imageTopic) {
      return;
    }
    ...
    // 先把 imageTopic 规范成带前导 / 的形式，便于和真实 topic 对齐：
    // - "medium_right" -> "/medium_right"
    // - "/medium_right" 保持不变
    const normalizedImageTopic = imageTopic.startsWith("/") ? imageTopic : `/${imageTopic}`;

    const prefix = getTopicMatchPrefix(normalizedImageTopic);

    // 用前缀优先，没有前缀时退化为 `startsWith("/medium_right/")` 这种判断
    const fallbackPrefix =
      prefix ??
      (normalizedImageTopic.endsWith("/") ? normalizedImageTopic : `${normalizedImageTopic}/`);

    for (const topic of annotationTopics) {
      const topicName = topic.name as TopicName;
      const currentSettings = config.annotations?.[topicName];

      const matchesByPrefix = fallbackPrefix !== "" && topicName.startsWith(fallbackPrefix);

      // 兼容极端情况：注释 topic 刚好等于 imageTopic 或 normalizedImageTopic
      const matchesExactly = topicName === imageTopic || topicName === normalizedImageTopic;

      const shouldBeVisible = matchesByPrefix || matchesExactly;

      if (currentSettings?.visible !== shouldBeVisible) {
        this.#handleTopicVisibilityChange(topicName, shouldBeVisible);
      }
    }
  }
}
